  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        // Only redirect to checkout if user never made a payment
        ...
        {status === "failed" ? (
          <>
            <span className={`${styles.icon} ${styles.iconWarn}`}>
              <ShieldAlert size={16} /> Payment Received / Under Verification
            </span>
            <h1>Payment Status & Instant Access</h1>
            <p className={styles.lead}>
              If money was debited from your bank account, <strong>your order is 100% safe!</strong> Click below to message us directly on WhatsApp and get your PDF immediately.
            </p>
            {orderId && <p className={styles.order}>Order ID: <strong>{orderId}</strong></p>}
            {paymentId && <p className={styles.order}>Payment ID: {paymentId}</p>}
            <a
              className={styles.whatsappBtn}
              href={`https://wa.me/919104826422?text=${encodeURIComponent(
                `Hi NokriMitra Support, I paid for OPD Mastery. Order ID: ${orderId || "N/A"}, Payment ID: ${paymentId || "N/A"}. Please send my PDF download link.`
              )}`}
              target="_blank"
          ...
          <>
            ...
            <a
              className={styles.backLink}
              style={{ color: "#25D366", marginTop: 12 }}
              href={`https://wa.me/919104826422?text=${encodeURIComponent(
                `Hi NokriMitra Support, I need help downloading my OPD Mastery PDF. Order ID: ${orderId || "N/A"}`
              )}`}
              target="_blank"
              rel="noopener noreferrer"
            >
