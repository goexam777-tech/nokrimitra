...
/**
 * Branded tokenized download route for the 500+ Human Anatomy Coloring Book
 * Bundle. e.g. https://nokrimitra.in/anatomy-coloring-book/go?t=<token>
 *
 * The token is issued only after a verified Cashfree payment, protecting the
 * direct Google Drive link while giving instant access to legitimate buyers.
 */
const COOKIE_NAME = "nm_anatomy_dl";
const SUPPORT_MAIL =
  "mailto:support@nokrimitra.in?subject=Anatomy%20Coloring%20Book%20download%20help";
...
export async function GET(req: Request) {
  ...
  const check = verifyDownloadToken("anatomy", token);
  ...
  const defaultDrive =
    "https://drive.google.com/drive/folders/1AHA6ohI2h3zfEG4BxE9wOTIS76-XQEdi?usp=sharing";
  const driveUrl = process.env.ANATOMY_DRIVE_URL || defaultDrive;
...
function blockedResponse(
  ...
  return statusPage({
    ...
    message: expired
      ? "Your Anatomy Coloring Book Bundle is still yours. Send us your Order ID and we will share a fresh link."
...
<body>
  <div class="box">
    ...
    <div class="body">
      ...
      <a class="back" href="/anatomy-coloring-book">Back to Anatomy Coloring Book</a>
    </div>
  </div>
</body>
</html>`,
