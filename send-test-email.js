// Script to send a test email using Resend API with the updated template
const fs = require('fs');
...
let apiKey = '';
let fromEmail = 'NokriMitra <onboarding@resend.dev>';

if (fs.existsSync(envPath)) {
  const envContent = fs.readFileSync(envPath, 'utf8');
  const apiKeyMatch = envContent.match(/^RESEND_API_KEY\s*=\s*["']?([^"'\r\n]+)["']?/m);
  if (apiKeyMatch) {
    apiKey = apiKeyMatch[1];
  }
  const fromMatch = envContent.match(/^EMAIL_FROM\s*=\s*["']?([^"'\r\n]+)["']?/m);
  if (fromMatch) {
    fromEmail = fromMatch[1];
  }
}

// Target email from command line args
const toEmail = process.argv[2];

if (!toEmail) {
  console.error("Please provide an email address, e.g.: node send-test-email.js your-email@gmail.com");
  process.exit(1);
}
...
const productName = "GSRTC કંડક્ટર સંપૂર્ણ PDF કોર્સ";
const customerName = "વિદ્યાર્થી";
const orderId = "order_mock_T8iTUqSD3aTmaV";
const amount = 1;
const downloadUrl = "https://pdf.nokrimitra.in/go";
const accent = "#0b6b3a";
const accentDark = "#084d2a";
const supportEmail = "support@nokrimitra.in";
const brandName = "NokriMitra.in";

// Build the email body matching lib/emailTemplate.ts
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.06);">
          ...
          <tr>
            <td style="padding:32px 28px 8px 28px;">
              <p style="margin:0 0 12px 0;font-size:16px;">પ્રિય <strong>${customerName}</strong>,</p>
